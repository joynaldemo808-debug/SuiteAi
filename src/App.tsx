...
import { TOOLS, Tool, ToolId, ResumeData, CVTemplate } from './types';
...
Languages: ${resumeData.skills.languages}
  ...
  const AIChat = () => (
    <div className="flex flex-col h-[calc(100vh-180px)] sm:h-[calc(100vh-200px)] max-w-4xl mx-auto bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden">
  ...
  const BottomNav = () => (
    <nav className="sm:hidden fixed bottom-0 left-0 right-0 z-50 px-4 pb-6 bg-gradient-to-t from-slate-50/80 dark:from-slate-950/80 to-transparent pointer-events-none">
      <div className="max-w-lg mx-auto bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border border-slate-200/50 dark:border-white/10 rounded-3xl shadow-2xl flex items-center justify-around p-2 pointer-events-auto">
        {[
          { id: 'home', icon: <LayoutDashboard className="w-5 h-5" />, label: t.home },
          { id: 'chat', icon: <MessageSquare className="w-5 h-5" />, label: t.aiChat },
          { id: 'history', icon: <History className="w-5 h-5" />, label: t.history },
          { id: 'profile', icon: <User className="w-5 h-5" />, label: t.profile },
        ].map((tab) => (
          <button
            key={tab.id}
            onClick={() => {
            ...
            className={`flex flex-col items-center gap-1 p-2 rounded-2xl transition-all relative flex-1 ${
              activeTab === tab.id 
                ? 'text-blue-600' 
  ...
  return (
    <ErrorBoundary>
      ...
      <AdPopup />
      {/* Header */}
      <header className="sticky top-0 z-50 glass border-b border-slate-200 dark:border-slate-800 px-4 py-2 sm:py-3">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div 
            className="flex items-center gap-2 sm:gap-3 cursor-pointer group"
            onClick={reset}
          >
            <div className="w-8 h-8 sm:w-10 sm:h-10 bg-blue-600 rounded-lg sm:rounded-xl flex items-center justify-center text-white shadow-lg shadow-blue-500/20 group-hover:scale-110 transition-transform">
              <Sparkles className="w-5 h-5 sm:w-6 sm:h-6" />
            </div>
            <div>
              <h1 className="text-lg sm:text-xl font-extrabold tracking-tight text-slate-900 dark:text-white">AI Suite</h1>
              <p className="text-[8px] sm:text-[10px] font-bold text-blue-600 uppercase tracking-widest">{t.poweredBy}</p>
            </div>
          </div>
          
          <div className="flex items-center gap-1 sm:gap-4">
            {user && userData && (
              <div className="hidden sm:flex items-center gap-2 sm:gap-3 px-2 sm:px-3 py-1 sm:py-1.5 bg-blue-50 dark:bg-blue-900/30 rounded-full border border-blue-100 dark:border-blue-800">
                <div className="flex items-center gap-1 sm:gap-1.5 text-blue-600 dark:text-blue-400">
                  ...
                  <span className="text-[10px] sm:text-xs font-bold">{userData.credits} {t.credits}</span>
                </div>
                {userData.isPremium && (
                  <span className="px-1.5 py-0.5 bg-amber-100 dark:bg-amber-900/50 text-amber-600 dark:text-amber-400 text-[8px] sm:text-[10px] font-bold rounded-full uppercase tracking-wider border border-amber-200 dark:border-amber-800">
                    Pro
                  </span>
                )}
              </div>
            ...
            <button
              ...
              className="p-1.5 sm:p-2 text-slate-600 dark:text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-slate-800 rounded-lg transition-all flex items-center gap-1.5 group/lang"
              title={t.language}
            >
              <Languages className="w-4 h-4 sm:w-5 h-5 transition-transform group-hover/lang:scale-110" />
              <span className="text-[10px] sm:text-xs font-bold hidden sm:inline">{language === 'en' ? 'বাংলা' : 'English'}</span>
            </button>

            <button
              type="button"
              onClick={() => setIsDarkMode(prev => !prev)}
              className="p-1.5 sm:p-2 text-slate-600 dark:text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-slate-800 rounded-lg transition-all group/theme"
            ...
            >
              <AnimatePresence mode="wait" initial={false}>
                ...
                >
                  {isDarkMode ? (
                    <Sun className="w-4 h-4 sm:w-5 h-5 transition-transform group-hover/theme:rotate-12" />
                  ) : (
                    <Moon className="w-4 h-4 sm:w-5 h-5 transition-transform group-hover/theme:-rotate-12" />
                  )}
                </motion.div>
              </AnimatePresence>
            </button>
            ...
            <button 
              ...
              className="hidden sm:flex p-2 text-slate-600 dark:text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-slate-800 rounded-lg transition-all group/history"
              title={t.history}
            >
            ...
            {user ? (
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full overflow-hidden border-2 border-blue-500 shadow-sm">
                  <img src={user.photoURL || ''} alt={user.displayName || ''} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                </div>
                <button
                  type="button"
                  onClick={handleLogout}
                  className="hidden sm:flex p-2 text-slate-600 dark:text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-all group/logout"
                  title={t.logout}
                >
              ...
              <button
                type="button"
                onClick={handleLogin}
                className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-blue-600 hover:bg-blue-700 text-white text-[10px] sm:text-sm font-bold rounded-lg sm:rounded-xl transition-all shadow-lg shadow-blue-500/20 active:scale-95 group/login"
              >
                <LogIn className="w-3.5 h-3.5 sm:w-4 h-4 transition-transform group-hover/login:translate-x-0.5" />
                <span>{t.login}</span>
              </button>
            )}
          </div>
      ...
      <main className="max-w-6xl mx-auto px-4 py-4 sm:py-8 relative flex-1">
        <AnimatePresence>
          {error && (
            <motion.div
          ...
          {activeTab === 'chat' ? (
                ...
                <div className="mb-8 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-white shadow-lg ${selectedTool?.color}`}>
                      {selectedTool && ICON_MAP[selectedTool.icon]}
                    </div>
                    <div>
            ...
            >
              <div className="col-span-full mb-6 sm:mb-8">
                <h2 className="text-2xl sm:text-4xl font-extrabold mb-2 sm:mb-3 tracking-tight text-slate-900 dark:text-white">{t.heroTitle}</h2>
                <p className="text-slate-600 dark:text-slate-400 text-sm sm:text-lg max-w-2xl">{t.heroSubtitle}</p>
              </div>
              {TOOLS.map((tool) => (
                <button
                  key={tool.id}
                  onClick={() => setSelectedToolId(tool.id)}
                  className="group relative bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-3xl sm:rounded-[2.5rem] border border-slate-200 dark:border-slate-800 text-left transition-all hover:shadow-2xl hover:shadow-blue-500/10 sm:hover:-translate-y-2 card-shadow overflow-hidden"
                >
                  <div className={`absolute top-0 right-0 w-24 sm:w-32 h-24 sm:h-32 bg-gradient-to-br ${tool.color} opacity-[0.03] rounded-bl-full transition-transform group-hover:scale-110`} />
                  
                  <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl flex items-center justify-center text-white shadow-lg mb-4 sm:mb-6 transition-transform group-hover:scale-110 group-hover:rotate-3 ${tool.color}`}>
                    {ICON_MAP[tool.icon]}
                  </div>
                  
                  <h3 className="text-lg sm:text-xl font-black mb-1 sm:mb-2 text-slate-900 dark:text-white group-hover:text-blue-600 transition-colors">
                  ...
                  <div className="mt-4 sm:mt-6 flex items-center gap-2 text-blue-600 font-bold text-[10px] sm:text-xs uppercase tracking-widest opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all">
                    {t.getStarted} <ChevronRight className="w-3.5 h-3.5 sm:w-4 h-4" />
                  </div>
                </button>
              ))}
            </motion.div>
