...
export default function App() {
  ...
  const [resumeData, setResumeData] = useState<ResumeData>(() => {
    ...
    return saved ? JSON.parse(saved) : {
      personal: {
        name: '', title: '', email: '', phone: '', address: '', summary: '', linkedin: '', github: '', website: '',
      },
  ...
  // Firebase Auth & Real-time Credits effect
  React.useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      if (firebaseUser) {
        ...
        if (!userDoc.exists()) {
          ...
          if (data.lastResetDate !== today && !data.isPremium) {
            try {
              ...
              setUserData(data); // এরর আসলেও পুরনো ডাটা সেট করে রাখা নিরাপদ
            }
          } else {
            setUserData(data);
          }
        ...
        // Real-time listener cleanup with safe variable (সমাধান ১)
        ...
        setUserData(null);
        // ইউজার লগআউট করলে ডকুমেন্ট লিসেনার বন্ধ করে দেওয়া হচ্ছে
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const canvas = await html2canvas(element, {
        ...
        onclone: (clonedDoc) => {
          // Empty loop removed to avoid unnecessary iterations (সমাধান ৪)
        }
      });
      ...
      // অপশনাল চেইনিং ব্যবহার করে ক্র্যাশ রোধ করা হয়েছে (সমাধান ৩)
      pdf.save(`${resumeData?.personal?.name || 'Resume'}_AI_CV.pdf`);
    } catch (error) {
      console.error('PDF generation failed:', error);
      setDownloadError(t?.pdfError || 'Failed to generate PDF.');
    } finally {
  ...
  const handleWatchAd = async () => {
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        // রিয়্যাক্ট স্টেট ব্যাচিং নিশ্চিত করতে এটি finally ব্লকে আনা হয়েছে
...
${resumeData.experience.map(exp => `- Role: ${exp.role} at ${exp.company} (${exp.dates})\n  Description: ${exp.description}`).join('\n')}



EDUCATION:

${resumeData.education.map(edu => `- ${edu.degree} from ${edu.institute} (${edu.year})`).join('\n')}



PROJECTS:

${resumeData.projects.map(proj => `- Name: ${proj.name}\n  Link: ${proj.link}\n  Description: ${proj.description}`).join('\n')}
...
INSTRUCTIONS: Improve the resume content provided above. Make the descriptions professional, action-oriented, and impactful. Ensure tone is appropriate for professional resume. Do not invent any facts that are not provided. Add a professional summary if not enough information is provided.
...
`;
  ...
  const handleResumeInputChange = (section: keyof ResumeData, field: string, value: string, index?: number) => {
    ...
    setResumeData(prev => {
      ...
      } else if (Array.isArray(newData[section]) && typeof index === 'number') {
          ...
          >
            ...
            <div className="flex flex-col gap-3">
              ...
              <button
                ...
                className="w-full py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-bold shadow-lg shadow-indigo-500/20 transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"
